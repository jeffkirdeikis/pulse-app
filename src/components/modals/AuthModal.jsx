  ...
  return (
    <div className="modal-overlay" role="dialog" aria-modal="true" aria-label="Sign in" onClick={handleClose}>
      <div className="auth-modal" ref={focusTrapRef} onClick={(e) => e.stopPropagation()}>
        <button className="auth-modal-close" onClick={handleClose} aria-label="Close"><X size={24} /></button>
        <div className="auth-modal-header">
          <div className="auth-logo">
            <MapPin size={32} />
          </div>
          <h2>{authMode === 'forgotPassword' ? 'Reset Password' : authMode === 'signin' ? 'Welcome Back' : 'Create Account'}</h2>
          <p>{authMode === 'forgotPassword' ? 'Enter your email and we\'ll send a reset link' : authMode === 'signin' ? 'Sign in to save events and connect with Squamish' : 'Join the Squamish community today'}</p>
        </div>
        <div className="auth-modal-body">
          {authMode !== 'forgotPassword' && (
            <>
              <button className="auth-btn google" onClick={async () => {
                const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.origin + window.location.pathname, queryParams: { prompt: 'select_account' } } });
                if (error) {
          ...
          <form onSubmit={authMode === 'forgotPassword' ? handleForgotPassword : authMode === 'signin' ? handleEmailSignIn : handleEmailSignUp} className="auth-form" noValidate>
            {authMode === 'signup' && (
            ...
            <button type="submit" className="auth-btn email" disabled={authLoading || (TURNSTILE_SITE_KEY && !captchaToken)}>
              <Mail size={20} />
              {authLoading ? 'Please wait...' : authMode === 'forgotPassword' ? 'Send Reset Link' : authMode === 'signin' ? 'Sign In' : 'Create Account'}
            </button>
          </form>
